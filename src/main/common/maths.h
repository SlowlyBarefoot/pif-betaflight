...
// core/pif.h reaches most translation units through scheduler.h and defines
// these too, guarded so whoever got there first keeps them. PIF's are the
// plain conditional form and evaluate their arguments twice; the ones below
// evaluate once, so they have to win wherever both headers are in scope. That
// is what the #undef is for - it is not an accidental redefinition.
#ifdef MIN
    #undef MIN
#endif
#define MIN(a,b) \
