...
void mspSerialAllocatePorts(void)
{
    ...
    while (portConfig && portIndex < MAX_MSP_PORT_COUNT) {
        ...
        if (serialPort) {
            pifUart_AttachTxTask(&serialPort->uart, PIF_ID_AUTO, TM_EXTERNAL, 0, "UART-TX-MSP");
            pifUart_AttachRxTask(&serialPort->uart, PIF_ID_AUTO, TM_PERIOD, 2000, "UART-RX-MSP");        // 2000ms
